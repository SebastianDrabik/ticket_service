    ...
    >
      ...
      {!readOnly && isHovering && onBlockDragStart && (
        <div
          ...
          className="absolute start-1 top-8 cursor-grab active:cursor-grabbing opacity-0 group-hover/table:opacity-100 transition-opacity duration-200 z-20"
      ...
      {!readOnly && isHovering && onDelete && (
        <div className="absolute end-0 top-2 z-20 opacity-0 group-hover/table:opacity-100 transition-opacity duration-200">
          <Button
            size="icon"
            variant="destructive"
            className="h-6 w-6"
      ...
      {/* Column controls - top */}
      {showColControls && (
        <div className="absolute top-0 start-8 end-0 flex justify-center gap-1 z-10">
      ...
      {showRowControls && bodyRows && (
        <div className="absolute -start-2 top-0 bottom-0 flex flex-col justify-start gap-1 z-10 pt-10">
          {bodyRows.map((_, idx) => (
            <div
              key={idx}
               ...
               <tr>
                 {headerRow.children.map((cell, colIdx) => (
                   <th
                     key={cell.id}
                     className={cn(
                       "border border-border bg-muted/50 p-2 font-semibold text-start relative group/cell",
                   ...
                   >
                     ...
                     {!readOnly && (
                       <div
                         className={cn(
                           "absolute end-0 top-0 bottom-0 w-1 cursor-col-resize hover:bg-primary/50 transition-colors",
                           resizingCol === colIdx && "bg-primary"
                         )}
                         onMouseDown={(e) => handleResizeStart(colIdx, e)}
                         style={{ userSelect: "none" }}
                       >
                         <div className="absolute end-0 top-1/2 -translate-y-1/2 -translate-x-1/2 rtl:translate-x-1/2 opacity-0 hover:opacity-100 transition-opacity">
